...
<!--         <div class="project_table_info">Stránka je optimalizována pro rozlišení s šířkou 1000px a větší.</div> -->
        ...
        <table id="mySearchTable" class="table search_project_table">
            <tr class="mySearchTable_header">
                ...
                <th class="headerIcon"> <a id="pdfBatchDownload" class="pdfDownloadLink" target="_blank" href="#" solutions=""> Stáhnout vybraná řešení <span class="pdfIcon headerIcon textCenter"><i class="far fa-file-pdf"></i> <span> </a> 
                </th>
            </tr>
            ...
            if ($results-> num_rows > 0 ) {
                while ($row = $results-> fetch_assoc()) {
                    ...
                    echo "<tr data-expanded='false' class='tableProjectRowStart'><td><strong> <a href='projecdetail.php?projectnumber=".$row["id"]."' target='_blank'>".$row["plny_nazev"]."</a></strong></td><td>".$rowEdit_plny_popis."</td> <td>".$row["kategorie"]."</td> <td>".$row["cilova_skupina"]."</td><td>".$row["souvisejici_kategorie"]."</td><td>". $rowEdit_vyuzitelne_produkty ."</td> <td> <input type='checkbox' id='type_" . $row["id"] . "' onclick='addTypeIDtoLink(" . $row["id"] . ")' name='Typ_". $row["id"] ."' solutionid='". $row["id"] ."' class='includeSolutionToPdf'> <label for='typ_". $row["id"] ."'> <span class='addToPDF'> <i class='fas fa-plus plusIcon addIcon'></i> <i class='fas fa-minus minusIcon addIcon displayNone'></i> <i class='far fa-file-pdf pdfIcon'></i> </span></label></td> </tr>";;
                }
                echo "";
            }
            else {
